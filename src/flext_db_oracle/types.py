"""Oracle database types and value objects - consolidated from duplications.

This module eliminates code duplication by importing classes from metadata.py
and creating backward-compatible aliases for existing TDbOracle* classes.

Architecture:
    This module now serves as a compatibility layer that imports the proper
    domain classes from metadata.py and provides aliases for backward compatibility.
    This eliminates the duplication between TDbOracle* and FlextDbOracle* classes.
"""
...
class CreateIndexConfig(FlextValueObject):
    ...
    def validate_business_rules(self) -> FlextResult[None]:
        ...
        try:
            ...
            elif any(not col or not col.strip() for col in self.columns):
...
class TDbOracleQueryResult(FlextValueObject):
    ...
    def get_column_values(self, column_name: str) -> FlextResult[list[object]]:
        ...
        values = [row[column_index] if column_index < len(row) else None
