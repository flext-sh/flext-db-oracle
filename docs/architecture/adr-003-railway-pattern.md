...
Implement Railway Pattern error handling throughout flext-db-oracle using r\[T\]:

**Core Pattern:**

```python
# Railway Pattern with r[T]
def enterprise_operation(config: Config) -> r[Result]:
    return (
...
- **No Bare Exceptions**: All operations return r, never throw exceptions
...
- **Memory Efficiency**: No exception t.NormalizedValue creation overhead
...
1. ✅ Adopt r[T] as the standard error handling pattern
1. ✅ Update all core APIs to return r instead of throwing exceptions
...
- ✅ All public APIs return r[T] instead of throwing exceptions
...
- [r Implementation](https://github.com/organization/flext/tree/main/flext-core/src/flext_core/result.py)
...
1. **Always Return r**: No exceptions thrown from business logic
