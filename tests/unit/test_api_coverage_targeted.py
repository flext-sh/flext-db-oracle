...
class TestAPIErrorHandlingPaths:
    ...
    def test_disconnected_status_path_lines_571_610(self, real_oracle_config) -> None:
        ...
        operations_to_test = [
            lambda: api.test_connection(),           # Should return disconnected status
            lambda: api.get_schemas(),              # Should handle not connected
            lambda: api.get_tables(),               # Should handle not connected
        ...
        for operation in operations_to_test:
            ...
            if result.is_success and hasattr(result, 'data') and isinstance(result.data, dict):
                data = result.data
                if "status" in data:
                    # Should indicate disconnected status (lines 574-580)
                    assert data.get("connected") is False or data.get("status") == "disconnected"

    def test_connection_error_handling_with_exceptions(self, real_oracle_config) -> None:
        ...
        with patch.object(api, '_connection') as mock_connection:
            ...
            mock_connection.test_connection.side_effect = Exception("Forced connection error")
    ...
    def test_api_internal_error_handling_paths(self, oracle_api, oracle_container) -> None:
        ...
        try:
            ...
            with patch.object(connected_api._connection, 'execute_query') as mock_execute:
...
class TestAPIQueryOptimization:
    ...
    def test_query_optimization_paths_lines_758_798(self, oracle_api, oracle_container) -> None:
    ...
    def test_query_error_handling_optimization(self, oracle_api, oracle_container) -> None:
...
class TestAPISchemaOperations:
    ...
    def test_schema_operations_error_handling_lines_1038_1058(self, oracle_api, oracle_container) -> None:
    ...
    def test_schema_operations_with_forced_errors(self, oracle_api, oracle_container) -> None:
        ...
        try:
            ...
            with patch.object(connected_api._connection, 'get_table_names') as mock_get_tables:
...
class TestAPIConnectionManagement:
    ...
    def test_connection_lifecycle_error_paths(self, real_oracle_config) -> None:
    ...
    def test_api_with_connection_state_changes(self, oracle_api, oracle_container) -> None:
...
class TestAPIObservabilityIntegration:
    ...
    def test_api_observability_paths(self, oracle_api, oracle_container) -> None:
        """Test API observability integration paths."""
