...
class TestsFlextDbOracleModels:
    ...
    def test_settings_accept_custom_values(self) -> None:
        ...
        settings = FlextDbOracleSettings.model_validate({"DbOracle": {
    ...
    def test_settings_value_equality(self) -> None:
        ...
        base = FlextDbOracleSettings().clone(DbOracle=FlextDbOracleSettings.DbOracleSettings.model_validate({"host": "localhost"}))
        same = FlextDbOracleSettings().clone(DbOracle=FlextDbOracleSettings.DbOracleSettings.model_validate({"host": "localhost"}))
        other = FlextDbOracleSettings().clone(DbOracle=FlextDbOracleSettings.DbOracleSettings.model_validate({"host": "remotehost"}))
    ...
    def test_settings_repr_includes_identifying_fields(self) -> None:
        ...
        settings = FlextDbOracleSettings.model_validate({"DbOracle": {"host": "localhost", "port": 1521, "username": "system"}})
