    ...
    def test_complete_oracle_workflow(
        ...
        with FlextDbOracleApi(real_oracle_config) as api:
            ...
            if connection_test.is_failure:
                raise TestOperationError("Connection", connection_test.error or "Unknown error")
            # Success case - connection is validated

            # Test schema discovery
            schemas_result = api.get_schemas()
            if schemas_result.is_failure:
                raise TestOperationError("Schema discovery", schemas_result.error or "Unknown error")
            ...
            if tables_result.is_failure:
                raise TestOperationError("Table listing", tables_result.error or "Unknown error")
            ...
            if create_ddl_result.is_failure:
                raise AssertionError(f"DDL generation failed: {create_ddl_result.error}")
            ddl_sql = create_ddl_result.value

            execute_result = api.execute_ddl(ddl_sql)  # nosec B608 # safe DDL generated by API
            ...
            try:
                ...
                for data in test_data:
                    ...
                    if insert_result.is_failure:
                        raise AssertionError(f"Data insertion failed: {insert_result.error}")
                ...
                if count_data and hasattr(count_data, "__getitem__") and len(count_data) > 0:
                    row = count_data[0]
                ...
                if metadata_result.is_failure:
                    raise AssertionError(f"Metadata query failed: {metadata_result.error}")
                ...
                if verify_result.is_failure:
                    raise AssertionError(f"Verification query failed: {verify_result.error}")
                email_value = verify_result.value
                if email_value and hasattr(email_value, "__getitem__") and len(email_value) > 0:
                    row = email_value[0]
                    if hasattr(row, "__getitem__") and hasattr(row, "__len__") and len(row) > 0:
