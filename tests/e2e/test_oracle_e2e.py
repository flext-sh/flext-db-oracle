...
class TestOracleE2E:
    ...
    def test_complete_oracle_workflow(
        ...
        with FlextDbOracleApi(real_oracle_config) as api:
            ...
            assert len(schemas_result.value) > 0, "No schemas found"

            # Test table listing
            tables_result = api.get_tables()
            assert tables_result.is_success, (
                f"Table listing failed: {tables_result.error}"
            )
            ...
            assert create_ddl_result.is_success, (
                f"DDL generation failed: {create_ddl_result.error}"
            )

            execute_result = api.execute_ddl(create_ddl_result.value)  # nosec B608 # safe DDL generated by API
            ...
            try:
                ...
                assert select_result.value.row_count == 3, (
                    f"Expected 3 rows, got {select_result.value.row_count}"
                )
                ...
                assert single_result.value[0][0] == 3, (
                    f"Expected count 3, got {single_result.value[0][0]}"
                )
                ...
                assert len(columns_result.value) >= 4

                # Test primary keys
                pk_result = api.get_primary_keys(test_table_name)
                assert pk_result.is_success, (
                    f"Primary key query failed: {pk_result.error}"
                )
                assert "ID" in pk_result.value, "ID should be primary key"  # nosec B608
                ...
                assert verify_result.value[0][0] == "bob@example.com"

            finally:
                # Cleanup: Drop test table
                drop_ddl_result = api.drop_table_ddl(test_table_name)
                if drop_ddl_result.is_success:
                    api.execute_ddl(drop_ddl_result.value)
        ...
        with FlextDbOracleApi(real_oracle_config) as api:
            ...
            for singer_type, expected_oracle_type in singer_types:
                ...
                assert expected_oracle_type in result.value, (
                    f"Expected {expected_oracle_type} in {result.value}"
                )
